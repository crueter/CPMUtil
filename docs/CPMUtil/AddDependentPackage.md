...
If Vulkan Headers are installed, but NOT Vulkan Utility Libraries, then CPMUtil will throw an error. Alternatively, if both are installed, but Vulkan Headers are too old, CPMUtil will also throw an error.